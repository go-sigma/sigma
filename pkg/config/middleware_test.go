...
func TestCheckRedis(t *testing.T) {
	// Disabled (no connection info) → no error
	err := checkRedis(Configuration{Redis: ConfigurationRedis{}})
	assert.NoError(t, err)

	// Valid URL → no error
	miniRedis := miniredis.RunT(t)
	err = checkRedis(Configuration{Redis: ConfigurationRedis{URL: "redis://" + miniRedis.Addr()}})
	assert.NoError(t, err)

	// Unreachable URL → error
	err = checkRedis(Configuration{Redis: ConfigurationRedis{URL: "redis://127.0.0.1:1100"}})
	assert.Error(t, err)
}
...
func TestCheckDatabase(t *testing.T) {
	...
	err = checkDatabase(Configuration{Database: ConfigurationDatabase{
		Type: enums.DatabaseTurso,
		Turso: ConfigurationDatabaseTurso{
			DSN: filepath.Join(t.TempDir(), "sigma.db"),
		},
	}})
	assert.NoError(t, err)
