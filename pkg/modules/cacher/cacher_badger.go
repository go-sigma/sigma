...
type badgerCacher[T any] struct {
	db      *badger.DB
	prefix  string
	fetcher Fetcher[T]
...
// newBadger returns a new Cacher.
func newBadger[T any](digCon *dig.Container, prefix string, fetcher Fetcher[T]) (Cacher[T], error) { // nolint: unparam
	return &badgerCacher[T]{
...
func (c *badgerCacher[T]) Set(ctx context.Context, key string, val T, ttls ...time.Duration) error {
...
func (c *badgerCacher[T]) Get(ctx context.Context, key string) (T, error) {
	var val T
	var result []byte
	err := c.db.View(func(txn *badger.Txn) error {
		item, err := txn.Get([]byte(genKey(c.config, c.prefix, key)))
		if err != nil {
			return err
		}
...
func (c *badgerCacher[T]) Del(ctx context.Context, key string) error {
	return c.db.Update(func(txn *badger.Txn) error {
		return txn.Delete([]byte(genKey(c.config, c.prefix, key)))
	})
}
