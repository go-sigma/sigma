...
func (c *clients) parseBlobRespHeader(header http.Header) (distribution.Descriptor, error) {
	size, err := strconv.ParseInt(header.Get(consts.HeaderContentLength), 10, 64)
	if err != nil {
		return distribution.Descriptor{}, fmt.Errorf("blob content length (%s) is invalid", header.Get(consts.HeaderContentLength))
	}
	...
	return distribution.Descriptor{
		MediaType: header.Get(consts.HeaderContentType),
		Size:      size,
		Digest:    digest,
	}, nil
}
...
func (c *clients) initUpload(ctx context.Context, repository string) (*url.URL, error) {
	...
	header.Add(consts.HeaderContentType, "application/octet-stream")
	...
	location := respHeader.Get(consts.HeaderLocation)
