...
//go:generate mockgen -destination=mocks/clients.go -package=mocks github.com/go-sigma/sigma/pkg/distribution/clients Clients
//go:generate mockgen -destination=mocks/clients_factory.go -package=mocks github.com/go-sigma/sigma/pkg/distribution/clients ClientsFactory
...
func (c clientsFactory) New(config *config.Configuration) (Clients, error) {
	...
	client.SetHeader(consts.HeaderUserAgent, consts.UserAgent)
...
func (c *clients) AuthToken() error {
	...
	if cha.Scheme == "bearer" {
		...
		_, err = c.ping(ctx, map[string]string{consts.HeaderAuthorization: fmt.Sprintf("Bearer %s", token)})
		if err != nil {
			return err
		}
		c.cli.SetHeader(consts.HeaderAuthorization, fmt.Sprintf("Bearer %s", token))
		return nil
	}
...
func (c *clients) ping(ctx context.Context, headers ...map[string]string) (*challenge.Challenge, error) {
	req := c.cli.R()
	if len(headers) > 0 {
	...
	authServerStr := resp.Header().Get(consts.HeaderWWWAuthenticate)
	if authServerStr == "" {
...
func (c *clients) token(ctx context.Context, cha challenge.Challenge) (string, error) {
	c.cli.Header().Del(consts.HeaderAuthorization) // clear the authorization header
	req := c.cli.R()
	req.SetContext(ctx)
	...
	if c.config.Proxy.Token != "" {
		req.SetHeader(consts.HeaderAuthorization, "Bearer "+c.config.Proxy.Token)
	}
	...
	var body api.PostUserTokenResponse
...
func (c *clients) DoRequest(ctx context.Context, method, path string, headers http.Header, bodyReaders ...io.Reader) (int, http.Header, io.ReadCloser, error) {
	...
	if headers != nil {
		...
		req.SetHeader(consts.HeaderContentType, "application/json")
		req.Header.Add(consts.HeaderAccept, "application/vnd.docker.distribution.manifest.v2+json")
		req.Header.Add(consts.HeaderAccept, "application/vnd.docker.distribution.manifest.list.v2+json")
		req.Header.Add(consts.HeaderAccept, "application/vnd.oci.image.index.v1+json")
		req.Header.Add(consts.HeaderAccept, "application/vnd.oci.image.manifest.v1+json")
	...
	slog.Info("clients do request", "url", url, "method", method, "header", req.Header)
