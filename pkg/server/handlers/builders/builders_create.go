...
func (h *handler) CreateBuilder(c echo.Context) error {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			...
			return errcode.NewHTTPError(c, errcode.HTTPErrCodeNotFound, "Repository not found")
		}
		log.Error().Err(err).Int64("id", req.RepositoryID).Msg("Repository find failed")
		return errcode.NewHTTPError(c, errcode.HTTPErrCodeInternalError, "Repository find failed")
	}
	...
	if err == nil {
		...
		return errcode.NewHTTPError(c, errcode.HTTPErrCodeConflict, "Repository has been already create builder")
	}
