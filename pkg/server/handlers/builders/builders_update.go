...
func (h *handler) UpdateBuilder(c *gin.Context, req *api.UpdateBuilderRequest) {
	...
	err := h.BuilderSvc.UpdateBuilder(ctx, user.ID, req.BuilderID, *req)
	if err != nil {
		if e, ok := errcode.AsType[errcode.ErrCode](err); ok {
			errcode.NewHTTPError(c, e)
			return
		}
		errcode.NewHTTPError(c, errcode.HTTPErrCodeInternalError, fmt.Sprintf("Update builder failed: %v", err))
		return
	}
