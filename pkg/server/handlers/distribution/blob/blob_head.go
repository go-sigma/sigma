...
func (h *handler) HeadBlob(c *gin.Context) {
	...
	blobObj, err := h.BlobSvc.HeadBlob(ctx, namespaceObj.ID, dgest.String(), c.Request.Method, c.Request.URL.Path)
	if err != nil {
		h.dsError(c, err)
		return
	}

	c.Writer.Header().Set(consts.HeaderContentLength, strconv.FormatInt(blobObj.Size, 10))
	c.Writer.Header().Set(consts.HeaderContentType, blobObj.ContentType)
	c.Writer.Header().Set(consts.ContentDigest, blobObj.Digest)
