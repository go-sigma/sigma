...
func (h *handler) Put(c echo.Context) error {
	...
	if req.Password != nil {
		...
		if err != nil {
			...
			return errcode.NewHTTPError(c, errcode.HTTPErrCodeInternalError, fmt.Sprintf("Hash password failed: %v", err))
		}
	...
	err = query.Q.Transaction(func(tx *query.Query) error {
		...
		if userObj.Role == enums.UserRoleAdmin {
			...
			if err != nil {
				...
				return errcode.HTTPErrCodeInternalError.Detail(fmt.Sprintf("Add role for user failed: %v", err))
			}
		} else {
			err = userService.DeletePlatformMember(ctx, userObj.ID, userObj.Role)
			if err != nil {
				log.Error().Err(err).Msg("Delete role for user failed")
				return errcode.HTTPErrCodeInternalError.Detail(fmt.Sprintf("Delete role for user failed: %v", err))
			}
		}
		return nil
	})
	if err != nil {
		...
		return errcode.NewHTTPError(c, errcode.HTTPErrCodeInternalError, fmt.Sprintf("Update user failed: %v", err))
	}
