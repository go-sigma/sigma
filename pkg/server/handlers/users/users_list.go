...
//	@Success	200				{object}	api.CommonList{items=[]api.UserItem}
//	@Failure	500				{object}	errcode.ErrCode
func (h *handler) List(c *gin.Context, req *api.GetUserListRequest) {
	...
	userObjs, total, err := h.UserSvc.ListUsers(ctx, exceptUsername, req.WithoutAdmin, req.Name, req.Pagination, req.Sortable)
	if err != nil {
		if e, ok := errcode.AsType[errcode.ErrCode](err); ok {
			errcode.NewHTTPError(c, e)
			return
		}
		slog.Error("list user failed", "err", err)
