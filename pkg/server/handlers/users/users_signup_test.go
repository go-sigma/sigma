...
// 	userHandler, err := handlerNew(inject{passwordSvc: passwordMock})
// 	assert.NoError(t, err)
...
// 	err := bootstrap.Initialize(config.Configuration{
// 		Auth: config.ConfigurationAuth{
// 			Admin: config.ConfigurationAuthAdmin{
// 				Username: "sigma",
// 				Password: "sigma",
...
// 	daoMockUserRepository := daomock.NewMockUserRepository(ctrl)
// 	daoMockUserRepository.EXPECT().GetByUsername(gomock.Any(), gomock.Any()).DoAndReturn(func(_ context.Context, _ string) (*models.User, error) {
// 		return nil, fmt.Errorf("test")
// 	}).Times(1)
// 	daoMockUserRepository.EXPECT().Create(gomock.Any(), gomock.Any()).DoAndReturn(func(_ context.Context, _ *models.User) error {
// 		return fmt.Errorf("test")
// 	}).Times(1)

// 	daoMockUserRepository := daomock.NewMockUserRepository(ctrl)
// 	daoMockUserRepository.EXPECT().New(gomock.Any()).DoAndReturn(func(txs ...*query.Query) repouser.UserRepository {
// 		return daoMockUserRepository
// 	}).Times(1)

// 	userHandler, err := handlerNew(inject{userRepository: daoMockUserRepository})
// 	assert.NoError(t, err)
