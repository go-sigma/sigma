...
func (h *handler) CreateRepository(c *gin.Context, req *api.CreateRepositoryRequest) {
	...
	repositoryObj, err := h.RepoSvc.CreateRepository(ctx, user.ID, *req)
	if err != nil {
		if e, ok := errcode.AsType[errcode.ErrCode](err); ok {
			errcode.NewHTTPError(c, e)
			return
		}
		errcode.NewHTTPError(c, errcode.HTTPErrCodeInternalError, fmt.Sprintf("Create repository failed: %v", err))
		return
	}

	c.JSON(http.StatusCreated, api.CreateRepositoryResponse{ID: repositoryObj.ID})
}
