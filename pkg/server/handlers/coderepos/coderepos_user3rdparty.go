...
//	@Success	200			{object}	api.GetCodeRepositoryUser3rdPartyResponse
//	@Failure	500			{object}	errcode.ErrCode
func (h *handler) User3rdParty(c *gin.Context, req *api.GetCodeRepositoryUser3rdPartyRequest) {
	...
	user3rdPartyObj, err := h.CodeRepoSvc.GetCodeRepositoryUser3rdParty(ctx, user.ID, req.Provider)
	if err != nil {
		if e, ok := errcode.AsType[errcode.ErrCode](err); ok {
			errcode.NewHTTPError(c, e)
			return
		}
		errcode.NewHTTPError(c, errcode.HTTPErrCodeNotFound, "Code repository find failed")
		return
	}

	c.JSON(http.StatusOK, api.GetCodeRepositoryUser3rdPartyResponse{
