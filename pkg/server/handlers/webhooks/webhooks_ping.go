...
func (h *handler) GetWebhookPing(c *gin.Context, req *api.GetWebhookPingRequest) {
	...
	err = h.WebhookSvc.PingWebhook(ctx, user.ID, req.WebhookID)
	if err != nil {
		if e, ok := errcode.AsType[errcode.ErrCode](err); ok {
			errcode.NewHTTPError(c, e)
			return
		}
		errcode.NewHTTPError(c, errcode.HTTPErrCodeInternalError, fmt.Sprintf("Ping webhook failed: %v", err))
		return
	}
