...
func (h *handler) GetWebhookLogResend(c *gin.Context, req *api.GetWebhookLogResendRequest) {
	...
	err = h.WebhookSvc.ResendWebhookLog(ctx, user.ID, req.WebhookID, req.WebhookLogID)
	if err != nil {
		if e, ok := errcode.AsType[errcode.ErrCode](err); ok {
			errcode.NewHTTPError(c, e)
			return
		}
		errcode.NewHTTPError(c, errcode.HTTPErrCodeInternalError, fmt.Sprintf("Resend webhook log failed: %v", err))
		return
	}
