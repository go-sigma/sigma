...
func (h *handler) GetWebhookLog(c echo.Context) error {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			...
			return errcode.NewHTTPError(c, errcode.HTTPErrCodeNotFound, fmt.Sprintf("Webhook log(%d) not found", req.WebhookLogID))
		}
		log.Error().Err(err).Int64("id", req.WebhookLogID).Msg("Get webhook log failed")
		return errcode.NewHTTPError(c, errcode.HTTPErrCodeInternalError, fmt.Sprintf("Get webhook log(%d) failed", req.WebhookLogID))
	}
