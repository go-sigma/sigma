...
func (h *handler) ListWebhook(c *gin.Context, req *api.ListWebhookRequest) {
	...
	webhookObjs, total, err := h.WebhookSvc.ListWebhooks(ctx, req.NamespaceID, req.Pagination, req.Sortable)
	if err != nil {
		if e, ok := errcode.AsType[errcode.ErrCode](err); ok {
			errcode.NewHTTPError(c, e)
			return
		}
		errcode.NewHTTPError(c, errcode.HTTPErrCodeInternalError, fmt.Sprintf("List webhook failed: %v", err))
		return
	}
	var resp = make([]any, 0, len(webhookObjs))
	for _, webhookObj := range webhookObjs {
		resp = append(resp, api.WebhookItem{
			ID:                webhookObj.ID,
