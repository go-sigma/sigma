...
func (h *handler) DeleteNamespace(c echo.Context) error {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			...
			return errcode.NewHTTPError(c, errcode.HTTPErrCodeNotFound, fmt.Sprintf("Namespace(%d) not found", req.ID))
		}
	...
	err = query.Q.Transaction(func(tx *query.Query) error {
		...
		if err != nil {
			...
			return errcode.HTTPErrCodeInternalError.Detail(fmt.Sprintf("Namespace(%d) find failed: %v", req.ID, err))
		}
		...
		if err != nil {
			...
			return errcode.HTTPErrCodeInternalError.Detail(fmt.Sprintf("Create audit for delete namespace failed: %v", err))
		}
