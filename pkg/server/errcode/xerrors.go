...
// NewHTTPError writes the given ErrCode as a JSON response.
func NewHTTPError(c *gin.Context, errCode ErrCode, message ...string) {
	if len(message) > 0 {
		errCode.Description = message[0]
	}
	c.JSON(errCode.HTTPStatusCode, errCode)
}
