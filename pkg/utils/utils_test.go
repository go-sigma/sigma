...
func TestGenPathByDigest(t *testing.T) {
	...
	assert.Equal(t, "08/e7/660f72aaa312f2ad1e13bc35afd988fa476052fd83296e0702e31ea00141", utils.GenPathByDigest(dgest))
	assert.Equal(t, "sigma/blobs/08/e7/660f72aaa312f2ad1e13bc35afd988fa476052fd83296e0702e31ea00141", utils.GenBlobPathByDigest(dgest))
	assert.Equal(t, "sigma/manifests/08/e7/660f72aaa312f2ad1e13bc35afd988fa476052fd83296e0702e31ea00141", utils.GenManifestPathByDigest(dgest))
}
...
func TestGetUserFromCtx(t *testing.T) {
	...
	tests := []struct {
		name     string
		format   utils.UserCtxErrorFormat
		...
		{
			name:   "normal",
			format: utils.UserCtxErrorHTTP,
			args: func(rec *httptest.ResponseRecorder) args {
				req := httptest.NewRequest(http.MethodPost, "/", nil)
				c, _ := gin.CreateTestContext(rec)
				c.Request = req
				c.Set(consts.ContextUser, &models.User{Username: "test", Password: new("test"), Email: new("test@gmail.com")})
				return args{c: c}
			},
			want:     &models.User{Username: "test", Password: new("test"), Email: new("test@gmail.com")},
			wantBool: false,
			respBody: "",
		},
		{
			name:   "http-missing-user",
			format: utils.UserCtxErrorHTTP,
			args: func(rec *httptest.ResponseRecorder) args {
				req := httptest.NewRequest(http.MethodPost, "/", nil)
				c, _ := gin.CreateTestContext(rec)
		...
		},
		{
			...
			format: utils.UserCtxErrorDistribution,
			args: func(rec *httptest.ResponseRecorder) args {
				req := httptest.NewRequest(http.MethodPost, "/", nil)
				c, _ := gin.CreateTestContext(rec)
				c.Request = req
		...
		{
			name:   "distribution-missing-user",
			format: utils.UserCtxErrorDistribution,
			args: func(rec *httptest.ResponseRecorder) args {
				req := httptest.NewRequest(http.MethodPost, "/", nil)
				c, _ := gin.CreateTestContext(rec)
		...
		{
			name:   "distribution-invalid-user",
			format: utils.UserCtxErrorDistribution,
			args: func(rec *httptest.ResponseRecorder) args {
				req := httptest.NewRequest(http.MethodPost, "/", nil)
				c, _ := gin.CreateTestContext(rec)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rec := httptest.NewRecorder()
			got, gotBool := utils.GetUserFromCtx(tt.args(rec).c, tt.format)
