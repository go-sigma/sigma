...
//go:generate go tool mockgen -destination=storage_mocks.go -package=storage github.com/go-sigma/sigma/pkg/storage StorageDriver
...
type StorageDriver interface {
	...
	// Ping verifies the storage backend is reachable and credentials are
	// valid. It writes a small probe object with a random suffix (to tolerate
	// multi-replica setups) and then deletes it; delete failures are ignored.
...
type Factory interface {
	New(config *config.Configuration) (StorageDriver, error)
}
...
func Initialize(config *config.Configuration) (StorageDriver, error) {
