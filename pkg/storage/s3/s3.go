...
func (f factory) New(cfg *config.Configuration) (storage.StorageDriver, error) {
	var optFns = []func(*awsconfig.LoadOptions) error{
		awsconfig.WithRegion(cfg.Storage.S3.Region),
		awsconfig.WithCredentialsProvider(aws.CredentialsProviderFunc(func(_ context.Context) (aws.Credentials, error) {
	...
	if !cfg.Storage.S3.OperateObjectWithoutMD5 {
		s3opts = append(s3opts, withContentMD5)
	}

	c, err := awsconfig.LoadDefaultConfig(context.Background(), optFns...)
	if err != nil {
...
func (a *awss3) Move(ctx context.Context, srcPath string, dstPath string) error {
	...
	if srcSize <= storage.MultipartCopyThresholdSize {
		_, e := a.client.CopyObject(ctx, &s3.CopyObjectInput{
			Bucket:     aws.String(a.bucket),
	...
	for i := range completedParts {
		...
		go func() {
			...
			uploadResp, e := a.client.UploadPartCopy(ctx, &s3.UploadPartCopyInput{
				Bucket:          aws.String(a.bucket),
				CopySource:      aws.String(path.Join(a.bucket, srcPath)),
				Key:             aws.String(dstPath),
...
func (a *awss3) Delete(ctx context.Context, path string) error {
	...
	var s3Objects = make([]s3types.ObjectIdentifier, 0, storage.MaxPaginationKeys)
	for paginator.HasMorePages() {
		...
		if len(s3Objects) > 0 {
			...
			if len(resp.Errors) > 0 {
				...
				errs := make([]s3types.Error, 0, len(resp.Errors))
...
func (a *awss3) UploadPart(ctx context.Context, path, uploadID string, partNumber int32, body io.Reader) (string, error) {
	...
	defer func() {
		...
		if e != nil {
			slog.Error("remove upload temp file failed", "err", e, "File", file.Name())
		}
	}()
...
// Ping verifies the S3 backend is reachable by writing and removing a small
// probe object. A random suffix avoids collisions under multi-replica setups.
// Delete failures are intentionally ignored.
func (a *awss3) Ping(ctx context.Context) error {
	probePath := path.Join(".sigma-healthz", "probe-"+uuid.NewV7String())
	if err := a.Upload(ctx, probePath, strings.NewReader("ping")); err != nil {
		return fmt.Errorf("s3 ping upload: %w", err)
	}
	_ = a.Delete(ctx, probePath)
	return nil
}
