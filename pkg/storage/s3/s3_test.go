...
func TestNewContentMD5Option(t *testing.T) {
	cfg := config.Configuration{
		Storage: config.ConfigurationStorage{
			S3: config.ConfigurationStorageS3{
				Region: "cn-north-1",
			},
		},
	}
	...
	var f factory
	driver, err := f.New(&cfg)
	assert.NoError(t, err)
	assert.Len(t, driver.(*awss3).opts, 1)

	cfg.Storage.S3.OperateObjectWithoutMD5 = true
	driver, err = f.New(&cfg)
	assert.NoError(t, err)
	assert.Empty(t, driver.(*awss3).opts)
}
