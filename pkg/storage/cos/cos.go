...
func (t *tencentcos) Move(ctx context.Context, srcPath, dstPath string) (err error) {
	...
	for i := range completedParts {
		go func() {
			...
			partResp, _, e := t.client.Object.CopyPart(ctx, dstPath, uploadID, i+1,
...
func (t *tencentcos) UploadPart(ctx context.Context, rPath, uploadID string, partNumber int32, body io.Reader) (string, error) {
	partPath := path.Join(storage.SanitizePath(t.rootDirectory, consts.BlobUploadParts),
		rPath, fmt.Sprintf("%s-%s", uploadID, uuid.NewV7String()))
...
// Ping verifies the COS backend is reachable by writing and removing a small
// probe object. A random suffix avoids collisions under multi-replica setups.
// Delete failures are intentionally ignored.
func (t *tencentcos) Ping(ctx context.Context) error {
	probePath := path.Join(".sigma-healthz", "probe-"+uuid.NewV7String())
	if err := t.Upload(ctx, probePath, strings.NewReader("ping")); err != nil {
		return fmt.Errorf("cos ping upload: %w", err)
	}
	_ = t.Delete(ctx, probePath)
	return nil
}
