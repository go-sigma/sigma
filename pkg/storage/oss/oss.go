...
func init() {
	utils.PanicIf(storage.Register(enums.StorageTypeOss, &factory{}))
}
...
func (a *alioss) Move(ctx context.Context, srcPath string, dstPath string) error {
	...
	srcSize, err := strconv.ParseInt(header.Get(consts.HeaderContentLength), 10, 64)
	if err != nil {
		slog.Error("get object header failed", "err", err, "Headers", header)
		return fmt.Errorf("convert header content-length to int failed: %v", err)
	}

	if srcSize <= storage.MultipartCopyThresholdSize {
		_, e := a.client.CopyObject(srcPath, dstPath)
		if e != nil {
			return e
		}
		return nil
	...
	for i := range completedParts {
		go func() {
			...
			lastByte := min(firstByte+storage.MultipartCopyChunkSize, srcSize)
			partSize := lastByte - firstByte
			uploadResp, e := a.client.UploadPartCopy(createResp, a.bucket, srcPath, firstByte, partSize, i+1)
			if e == nil {
