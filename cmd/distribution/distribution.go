...
import (
	...
	"github.com/go-sigma/sigma/pkg/app/distribution"
...
func NewCmdDistribution() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(_ *cobra.Command, _ []string) {
			...
			if err != nil {
				slog.Error("initialize database with error", "err", err)
				return
			}

			err = bootstrap.Initialize(digCon)
			if err != nil {
				slog.Error("initialize bootstrap with error", "err", err)
				return
			}

			err = distribution.Serve(digCon)
			if err != nil {
				slog.Error("start distribution with error", "err", err)
				return
			}
		},
	}
