...
func Execute() {
	...
	if err != nil {
		slog.Error("execute root command failed", "err", err)
		os.Exit(1)
	}
}
...
func initConfig() {
	...
	if !utils.IsExist(cfgFile) {
		logger.Fatal("config file not exist", "config", cfgFile)
	}
	data, err := os.ReadFile(cfgFile)
	if err != nil {
		logger.Fatal("read config file failed", "err", err)
	}
	cfg := config.GetConfig()
	err = yaml.Unmarshal(data, cfg)
	if err != nil {
		logger.Fatal("unmarshal failed", "err", err)
	}
	err = cfg.ApplyEnvOverrides()
	if err != nil {
		logger.Fatal("apply env config overrides failed", "err", err)
	}
	logger.SetLevel(cfg.Log.Level.String())
}
