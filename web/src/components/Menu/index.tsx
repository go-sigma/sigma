...
const menuList: IMenu[] = [
  {
    ...
    url: "home",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" fill="currentColor" className="h-6 w-6">
            ...
            <div className="space-y-1">
              {
                menuList.map(m => {
                  return (
                    <Link to={`/${m.url.toLowerCase()}`} key={m.text} className={`text-gray-700 group flex items-center px-2 py-2 text-sm font-medium rounded-md ${menuActive === m.text ? "bg-gray-100" : "hover:bg-gray-100 text-gray-700"}`} onClick={() => {
